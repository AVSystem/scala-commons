...
* Any Scala collection extending `scala.collection.Seq[T]` or `scala.collection.Set[T]`, provided that `T` can be serialized
  and there is an appropriate instance of `BuildFrom` (e.g. `GenCodec[List[T]]` requires `Factory[T, List[T]]`).
  All standard library collections have this `BuildFrom` instance so you only have to worry about it when dealing with
...
* Any `scala.collection.Map[K,V]` provided that `V` can be serialized and there is an implicit
  ...
  so that it can be converted to a string and used as object key. There must also be an appropriate instance of `BuildFrom` for
  particular `Map` implementation (e.g. `HashMap[K,V]` requires `Factory[(K, V), HashMap[K, V]]`).
  All standard library map implementations have this `BuildFrom` instance so you only have to worry about it when dealing
