...
When changing definitions of serialized data types, you must ensure that your changes don't break compatibility. `GenCodec` guarantees that some changes are safe
while other are not.
...
Case classes in a sealed hierarchy may be changed in the same way as if they were standalone classes (except for renaming).
