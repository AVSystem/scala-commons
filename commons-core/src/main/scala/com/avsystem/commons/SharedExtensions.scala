...
object SharedExtensions extends SharedExtensions {
  ...
  def traverseCompleted[A, B, M[X] <: TraversableOnce[X]](in: M[A])(fn: A => Future[B])(
    implicit cbf: CanBuildFrom[M[A], Try[B], M[Try[B]]],
    cbf2: CanBuildFrom[M[Try[B]], B, M[B]],
    executor: ExecutionContext
  ): Future[M[B]] =
    Future.traverse(in)(el => fn(el).transformNow(Success(_))).mapNow(tries => Try.sequence(tries).get)
