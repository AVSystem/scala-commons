...
object RestResponse {
  ...
  implicit class AsyncOps(private val asyncResp: Async[RestResponse]) extends AnyVal {
    def recoverHttpError: Async[RestResponse] =
      callback => asyncResp {
  ...
  ): AsReal[Async[RestResponse], Try[F[T]]] =
    AsReal.create(async => Success(fromAsync.fromAsync(Async.map(async)(resp => asResponse.asReal(resp)))))

  implicit def effectToAsyncResp[F[_], T](
    implicit toAsync: RawRest.ToAsync[F], asResponse: AsRaw[RestResponse, T]
  ): AsRaw[Async[RestResponse], Try[F[T]]] =
    AsRaw.create(_.fold(
      Async.failed,
      ft => Async.map(toAsync.toAsync(ft))(asResponse.asRaw)
    ).recoverHttpError)
  ...
  ): ImplicitNotFound[AsReal[Async[RestResponse], Try[F[T]]]] = ImplicitNotFound()
  ...
  ): ImplicitNotFound[AsRaw[Async[RestResponse], Try[F[T]]]] = ImplicitNotFound()
