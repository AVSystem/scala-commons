...
case class Stuff(map: Map[String, Boolean], factor: Double) derives GenCodec

@flatten sealed trait FlatSealedStuff derives GenCodec
sealed trait SealedStuff derives GenCodec
...
case class Foo(s: String, d: Double, i: Int, l: Long, bs: List[Boolean]) derives GenCodec
@nowarn
object Foo {
