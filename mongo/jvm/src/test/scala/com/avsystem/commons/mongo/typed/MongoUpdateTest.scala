...
class MongoUpdateTest extends AnyFunSuite {
  ...
  test("simple operators") {
    assert(toString(Rte.ref(_.int).set(5)) == """{"$set": {"int": 5}}, []""")
    assert(toString(Rte.ref(_.int).setOnInsert(5)) == """{"$setOnInsert": {"int": 5}}, []""")
    assert(toString(Rte.ref(_.int).inc(5)) == """{"$inc": {"int": 5}}, []""")
    assert(toString(Rte.ref(_.int).min(5)) == """{"$min": {"int": 5}}, []""")
    assert(toString(Rte.ref(_.int).max(5)) == """{"$max": {"int": 5}}, []""")
    assert(toString(Rte.ref(_.int).mul(5)) == """{"$mul": {"int": 5}}, []""")
