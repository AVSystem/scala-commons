...
object `package` {
  ...
  /** A callback that gets notified when value of type `T` gets computed or when computation of that value fails.
    * Callbacks should never throw exceptions. Preferably, they should be simple notifiers that delegate the real work
    * somewhere else, e.g. schedule some handling code on a separate executor (e.g.
    * [[scala.concurrent.ExecutionContext ExecutionContext]]).
    */
  type Callback[T] = Try[T] => Unit

  /** The most low-level, raw type representing an asynchronous, possibly side-effecting operation that yields a value
    * of type `T` as a result. `Async` is a consumer of a callback. When a callback is passed to `Async`, it should
    * start the operation and ultimately notify the callback about the result. Each time the callback is passed, the
    ...
    * Implementation of `Async` should also be prepared to accept a callback before the previous one was notified about
    * the result (i.e. it should support concurrent execution).
    */
