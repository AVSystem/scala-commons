 ...
 * }}}
 *
 * `GenCodec.write[Base](someOutput, Something(42))` would write an object `{"STH": {"dbname": 42}}` instead of
 * `{"Something": {"paramname": 42}}`.
 *
 * NOTE: `@name` annotation may be defined on any level of inheritance hierarchy. For instance, if a case class field
