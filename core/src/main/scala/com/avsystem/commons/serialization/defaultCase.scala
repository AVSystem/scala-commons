...
/** When materializing a `GenCodec` for sealed hierarchy with `@flatten` annotation, you can use this annotation on one
  ...
  * `_case` field and determine the case class/object to deserialize, it will try to deserialize the data to the
  * class/object marked with this annotation.
  *
  * This is useful for retaining backwards compatibility with serialized format when refactoring code and replacing a
  * simple case class with a sealed hierarchy.
  *
  * @param transient
