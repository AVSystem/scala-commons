 ...
 * parameterless (no parameter lists or empty parameter list) or accept only implicit parameters, provided that all the
 * implicit values are available in the scope where `GenCodec` is materialized (implicit values will be "baked in" the
 * codec).
 *
