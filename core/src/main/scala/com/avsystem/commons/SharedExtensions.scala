...
import com.avsystem.commons.SharedExtensions.{FutureCompanionOps, IteratorCompanionOps, TryCompanionOps}
...
trait SharedExtensions {
  ...
  extension [A](fut: Future[A]) {
    ...
    def ignoreFailures: Future[Unit] =
      fut.thenReturn(Future.successful {})
  }
  ...
  extension (companion: Future.type) {
    ...
    /** Evaluates an expression and wraps its value into a `Future`. Failed `Future` is returned if expression
      ...
      */
    def eval[T](expr: => T): Future[T] = FutureCompanionOps.eval(expr)

    /** Different version of `Future.traverse`. Transforms a `IterableOnce[A]` into a `Future[IterableOnce[B]]`, which
      * only completes after all `in` `Future`s are completed, using the provided function `A => Future[B]`.
      */
    ...
    ): Future[M[B]] = FutureCompanionOps.traverseCompleted(in)(fn)
    ...
    ): Future[M[A]] = FutureCompanionOps.sequenceCompleted(in)
  }
  ...
  extension (companion: Try.type) {
    ...
    def traverse[A, B, M[X] <: IterableOnce[X]](in: M[A])(fn: A => Try[B])(implicit bf: BuildFrom[M[A], B, M[B]])
      : Try[M[B]] = TryCompanionOps.traverse(in)(fn)
  }
  ...
  extension [C[X] <: IterableOnce[X], A](coll: C[A]) {
    ...
    def findOpt(p: A => Boolean): Opt[A] = coll.iterator.find(p).toOpt
    ...
    def collectFirstOpt[B](pf: PartialFunction[A, B]): Opt[B] = coll.iterator.collectFirst(pf).toOpt
    ...
    def maxOpt(implicit ord: Ordering[A]): Opt[A] = {
      val it = coll.iterator
      if (it.isEmpty) Opt.Empty else it.max.opt
    }

    def maxOptBy[B: Ordering](f: A => B): Opt[A] = {
      val it = coll.iterator
      if (it.isEmpty) Opt.Empty else it.maxBy(f).opt
    }

    def minOpt(implicit ord: Ordering[A]): Opt[A] = {
      val it = coll.iterator
      if (it.isEmpty) Opt.Empty else it.min.opt
    }

    def minOptBy[B: Ordering](f: A => B): Opt[A] = {
      val it = coll.iterator
  ...
  extension (companion: Iterator.type) {
    def untilEmpty[T](elem: => Opt[T]): Iterator[T] = IteratorCompanionOps.untilEmpty(elem)

    def iterateUntilEmpty[T](start: Opt[T])(nextFun: T => Opt[T]): Iterator[T] =
      IteratorCompanionOps.iterateUntilEmpty(start)(nextFun)
  }
...
object SharedExtensions extends SharedExtensions {
  ...
  object PartialFunctionOps {
    ...
    private[commons] final val NoValueMarkerFunc = (_: Any) => NoValueMarker
  }
