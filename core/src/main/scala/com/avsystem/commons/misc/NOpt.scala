...
final class NOpt[+A] @publicInBinary private[misc] (@publicInBinary private[misc] val rawValue: Any)
  ...
  def boxed[B](implicit boxing: Boxing[A, B]): NOpt[B] =
    map(boxing.fun)

  def unboxed[B](implicit unboxing: Unboxing[B, A]): NOpt[B] =
    map(unboxing.fun)
  ...
  def flatten[B](implicit ev: A <:< NOpt[B]): NOpt[B] =
  ...
  def zip[B](that: NOpt[B]): NOpt[(A, B)] =
