...
object Opt {
  ...
  final class LazyOptOps[A](private val opt: () => Opt[A]) extends AnyVal {
    ...
    /** Unless a given condition is true, this will evaluate the `opt` argument and return it. Otherwise, `opt` is not
      ...
      */
    def unless(cond: Boolean): Opt[A] = when(!cond)
  }
...
final class Opt[+A] @publicInBinary private[misc] (@publicInBinary private[misc] val rawValue: Any)
  ...
  def boxed[B](implicit boxing: Boxing[A, B]): Opt[B] =
    map(boxing.fun)

  def boxedOrNull[B >: Null](implicit boxing: Boxing[A, B]): B =
    if (isEmpty) null else boxing.fun(value)

  def unboxed[B](implicit unboxing: Unboxing[B, A]): Opt[B] =
    map(unboxing.fun)
  ...
  def flatten[B](implicit ev: A <:< Opt[B]): Opt[B] =
