 ...
 * to traditional Scala approach of using a sealed hierarchy with objects representing enum values.
 *
 * Advantages of value based enums over object based enums include: <ul> <li>Much less classes generated, which in
 * particular contributes to much less JavaScript output in ScalaJS. This may also speed up compilation.</li> <li>No
 * need to explicitly implement `values` in enum's companion object as it is necessary when using
 * [[SealedEnumCompanion]] and [[NamedEnumCompanion]]</li> <li>It is possible to define all enum values in a single
 * line of code (assuming they don't take parameters)</li> </ul> Disadvantages of value based enums over object based
 * enums include: <ul> <li>Every object can have its own separate public API, values cannot (although you can have a
 * separate anonymous class for every value)</li> <li>Scala compiler does not perform exhaustive checking for pattern
 * matching enum values - this is however provided separately by `commons-analyzer` compiler plugin.</li> </ul>
 *
 ...
 * companion object and must have the `Value` type explicitly ascribed (which is just a type alias for enum class
 * itself). The enum class itself must take an implicit [[EnumCtx]] argument which provides information about enum
 * ordinal and name as well as makes sure that enum value is registered in the companion object. If possible, you
 * should always extend [[AbstractValueEnum]] instead of mixing in this trait. [[ValueEnum]] trait should only be mixed
 * in directly when your enum class already has another superclass, incompatible with [[AbstractValueEnum]].
 *
 * @example
 *   {{{
  ...
  final lazy val values: IIndexedSeq[T] = synchronized {
    if (awaitingRegister) {
      throw new IllegalStateException(
        s"Cannot collect enum values - one of the created contexts didn't register a value yet",
      )
    }
    finished = true
 ...
 * better binary compatibility it's better to use this abstract class rather than [[ValueEnumCompanion]] trait
 * directly. See [[ValueEnum]] documentation for more information on value based enums.
 */
