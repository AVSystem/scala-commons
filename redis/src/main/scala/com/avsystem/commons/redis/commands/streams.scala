...
trait StreamsApi extends ApiSubset {
  ...
  def xgroupCreate(key: Key, group: XGroup, id: OptArg[XEntryId] = OptArg.Empty, mkstream: Boolean = false)
    : Result[Unit] =
  ...
  def xreadgroupSingle(
    ...
    execute(
      new Xreadgroup(group, consumer, count.toOpt, blockMillis.toOpt, noack, Iterator(key), Iterator(id.toOpt))
        .map(_.getOrElse(key, Nil))
    )
  ...
  def xreadgroup(
    ...
    execute(
      new Xreadgroup(
        ...
        blockMillis.toOpt,
        noack,
        streams.iterator.map(_._1),
        streams.iterator.map(_._2),
      )
    )
  ...
  private final class Xack(key: Key, group: XGroup, ids: Iterable[XEntryId]) extends RedisIntCommand with NodeCommand {
  ...
  private final class Xadd(key: Key, maxlen: Opt[XMaxlen], id: Opt[XEntryId], data: Record)
    ...
    val encoded: Encoded = encoder("XADD").key(key).optAdd("MAXLEN", maxlen).optAdd(id, "*").dataPairs(data).result
  }

  private abstract class AbstractXclaim[A](
    entryDecoder: ReplyDecoder[A]
  )(
    key: Key,
    group: XGroup,
    consumer: XConsumer,
    ...
    force: Boolean,
    justid: Boolean,
  ) extends AbstractRedisCommand[Seq[A]](multiBulkAsSeq(entryDecoder))
      with NodeCommand {
    val encoded: Encoded = encoder("XCLAIM")
      .key(key)
      .add(group)
      .add(consumer)
      .add(minIdleTime)
      .add(ids)
      .optAdd("IDLE", idle)
      .optAdd("TIME", msUnixTime)
      .optAdd("RETRYCOUNT", retrycount)
      .addFlag("FORCE", force)
      .addFlag("JUSTID", justid)
      .result
  ...
  private final class XgroupCreate(key: Key, group: XGroup, id: Opt[XEntryId], mkstream: Boolean)
    ...
    val encoded: Encoded =
      encoder("XGROUP", "CREATE").key(key).add(group).optAdd(id, "$").addFlag("MKSTREAM", mkstream).result
  }
  ...
  private final class XgroupDestroy(key: Key, group: XGroup) extends RedisBooleanCommand with NodeCommand {
  ...
  private final class XpendingEntries(
    key: Key,
    group: XGroup,
    start: Opt[XEntryId],
    end: Opt[XEntryId],
    count: Int,
    consumer: Opt[XConsumer],
  ) extends RedisSeqCommand[XPendingEntry](multiBulkAsXPendingEntry)
      with NodeCommand {
    val encoded: Encoded =
      encoder("XPENDING").key(key).add(group).optAdd(start, "-").optAdd(end, "+").add(count).optAdd(consumer).result
  }
  ...
  private final class Xrange(key: Key, start: Opt[XEntryId], end: Opt[XEntryId], count: Opt[Int])
    ...
    val encoded: Encoded = encoder("XRANGE").key(key).optAdd(start, "-").optAdd(end, "+").optAdd("COUNT", count).result
  }

  private abstract class AbstractXread(noStreams: Boolean)
    extends AbstractRedisCommand[BMap[Key, Seq[XEntry]]](multiBulkAsXEntriesMapOf[Key, Record]) with NodeCommand {
  ...
  private final class Xread(
    count: Opt[Int],
    val blockMillis: Opt[Int],
    streamKeys: Iterator[Key],
    streamIds: Iterator[Opt[XEntryId]],
  ) extends AbstractXread(streamKeys.isEmpty) {
    val encoded: Encoded = encoder("XREAD")
      .optAdd("COUNT", count)
      .optAdd("BLOCK", blockMillis)
      .add("STREAMS")
      .keys(streamKeys)
      .add(streamIds.map(_.fold("$")(_.toString)))
      .result
  }
  ...
  private final class Xrevrange(key: Key, end: Opt[XEntryId], start: Opt[XEntryId], count: Opt[Int])
    extends RedisSeqCommand[XEntry](multiBulkAsXEntryOf[Record]) with NodeCommand {
    val encoded: Encoded =
      encoder("XREVRANGE").key(key).optAdd(end, "+").optAdd(start, "-").optAdd("COUNT", count).result
  }
...
object XMaxlen {
  implicit val commandArg: CommandArg[XMaxlen] = CommandArg { case (enc, XMaxlen(maxlen, approx)) =>
    enc.addFlag("~", approx).add(maxlen)
  }
}
