...
trait SetsApi extends ApiSubset {
  ...
  /** Executes [[http://redis.io/commands/sadd SADD]] or simply returns 0 when `members` is empty
    */
  ...
  /** Executes [[http://redis.io/commands/sinter SINTER]] NOTE: `keys` MUST NOT be empty
    */
  ...
  /** Executes [[http://redis.io/commands/sinterstore SINTERSTORE]] NOTE: `keys` MUST NOT be empty
    */
  ...
  /** Executes [[http://redis.io/commands/srem SREM]] or simply returns 0 when `members` is empty
    */
  ...
  def sscan(key: Key, cursor: Cursor, matchPattern: OptArg[Value] = OptArg.Empty, count: OptArg[Int] = OptArg.Empty)
    : Result[(Cursor, Seq[Value])] =
  ...
  /** Executes [[http://redis.io/commands/sunion SUNION]] or simply returns empty set when `keys` is empty
    */
  ...
  /** Executes [[http://redis.io/commands/sunionstore SUNIONSTORE]] NOTE: unlike in `SUNION` `keys` MUST NOT be empty
    * because `SUNIONSTORE` overwrites destination key. An operation equivalent to invoking `SUNIONSTORE` with only
    * `destination` and no `keys` would be `DEL destination`
    */
  ...
  private final class Sdiffstore(destination: Key, source: Key, keys: Iterable[Key])
  ...
  private final class Smismember(key: Key, members: Iterable[Value])
  ...
  private final class Sscan(key: Key, cursor: Cursor, matchPattern: Opt[Value], count: Opt[Int])
    ...
    val encoded: Encoded =
      encoder("SSCAN").key(key).add(cursor.raw).optData("MATCH", matchPattern).optAdd("COUNT", count).result
  }
