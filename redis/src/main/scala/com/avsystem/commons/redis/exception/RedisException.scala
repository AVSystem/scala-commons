...
/** Throw when response sent back by Redis server is corrupt according to Redis protocol.
  */
class InvalidDataException(msg: String) extends RedisException(msg)

/** Thrown when transactions fails due to one of watched keys having been modified by another client, i.e. when `EXEC`
  * command in a `WATCH`-`MULTI`-`EXEC` transaction returns null bulk response.
  */
class OptimisticLockException extends RedisException("One of the watched keys has been modified by another client")
...
/** Command or operation is failed with this exception when it has been already sent through network but the connection
  * was closed before receiving a response. Even though connections are automatically restarted, such command cannot be
  * resent because we don't know whether it was actually executed on the Redis instance or not. In a Redis Cluster
  * deployment, this is likely to happen when a node fails.
  */
...
/** Thrown when some command was queued for execution on a
  * [[com.avsystem.commons.redis.RedisNodeClient RedisNodeClient]] connected to one of the master nodes in Redis Cluster
  * and [[com.avsystem.commons.redis.RedisClusterClient RedisClusterClient]] detected that this node is no longer a
  * master before the command could be sent to it.
  */
...
class NodeInitializationFailure(cause: Throwable) extends RedisException(s"Failed to initialize node client", cause)

/** Thrown when trying to execute command unsupported by particular client type. For example, it's impossible to execute
  * connection state changing commands like `CLIENT SETNAME` using a
...
/** Thrown when there is too many concurrent blocking commands being executed on a node client and blocking connection
  * pool is exhausted.
  */
...
/** Thrown when [[com.avsystem.commons.redis.RedisClusterClient RedisClusterClient]] is unable to fetch initial cluster
  * state from any of the seed nodes. This happens e.g. when none of the seed nodes can be contacted or when they aren't
  * Redis Cluster members.
  *
  * @param seedNodes
  *   seed node addresses passed to [[com.avsystem.commons.redis.RedisClusterClient RedisClusterClient]]
  */
...
/** Thrown when [[com.avsystem.commons.redis.RedisMasterSlaveClient RedisMasterSlaveClient]] is unable to fetch current
  * master address from any of the sentinel seed nodes.
  *
  * @param masterName
  *   name of the master monitored by sentinels
  * @param seedSentiels
  *   seed sentinel addresses passed to [[com.avsystem.commons.redis.RedisMasterSlaveClient RedisMasterSlaveClient]]
  */
class MasterSlaveInitializationException(val masterName: String, val seedSentiels: Seq[NodeAddress])
  extends RedisException(
    s"Failed to fetch master node for $masterName from all of the seed sentinels: ${seedSentiels.mkString(", ")}"
  )
...
/** Thrown when some multi-keyed command or `MULTI`-`EXEC` block executed by
  ...
  */
class CrossSlotException extends RedisException("Keys don't hash to the same slot")

/** Throw when (according to current cluster state) some slot is not served by any master. This most likely indicates
  * problem with setup of the Redis Cluster deployment itself.
  */
class UnmappedSlotException(val slot: Int) extends RedisException(s"Slot $slot is not served by any node")
...
/** Thrown when trying to execute command or `MULTI`-`EXEC` block that does not contain any keys using
  ...
  */
class NoKeysException extends RedisException(s"Cannot execute commands with no keys using cluster client")

/** Thrown when too many consecutive cluster redirections occurred during execution of some command by
  * [[com.avsystem.commons.redis.RedisClusterClient RedisClusterClient]]. This might indicate a problem with
  * configuration of Redis Cluster deployment itself.
  *
...
class TooManyRedirectionsException(val lastRedirection: Redirection)
  extends RedisException(
    s"Too many Redis cluster redirections, last one to: " +
