...
final class RedisConnectionActor(
  ...
  private def connecting(retryStrategy: RetryStrategy, readInitSender: Opt[ActorRef]): Receive = {
    ...
    case Connect =>
      ...
      doConnect()
    case Connected(connection, remoteAddress, localAddress, connectionId) =>
      if (currentConnectionId.forall(_ != connectionId)) {
  ...
  private def doConnect(): Unit = {
    ...
    connFuture.onCompleteNow {
      case Success(Tcp.OutgoingConnection(remoteAddress, localAddress)) =>
        self ! Connected(actorRef, remoteAddress, localAddress, connectionId)
      case Failure(cause) =>
  ...
  private final class ConnectedTo(connection: ActorRef, localAddr: InetSocketAddress, remoteAddr: InetSocketAddress)
    ...
    def become(receive: Receive): Unit =
      context.become(receive.unless {
        case Connected(oldConnection, _, _, _) if oldConnection != connection =>
    ...
    def initialize(retryStrategy: RetryStrategy): Unit = {
      ...
      new ReplyCollector(initBatch.rawCommandPacks, initBuffer, onInitResult(_, retryStrategy)).sendEmptyReplyOr {
        collector =>
          flip(initBuffer)
      ...
      if (!waitingForAck && !queuedToWrite.isEmpty) {
        ...
        while (packsCount > 0) {
          ...
          queued.packs match {
            ...
            case packs =>
              new ReplyCollector(packs, writeBuffer, queued.reply).sendEmptyReplyOr(collectors.addLast)
          }
        }
