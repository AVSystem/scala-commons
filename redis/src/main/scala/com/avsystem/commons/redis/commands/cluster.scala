...
trait NodeClusterApi extends KeyedClusterApi {
  ...
  private object ClusterBumpepoch
    extends AbstractRedisCommand[BumpepochResult](simpleBumpepochResult) with NodeCommand {
  ...
  private final class ClusterReplicas(nodeId: NodeId)
    extends AbstractRedisCommand[Seq[NodeInfo]](multiBulkAsNodeInfos) with NodeCommand {
...
class NodeFlags(val raw: Int) extends AnyVal {
  ...
  override def toString: String =
    ...
    else reprValuePairs.iterator.collect { case (str, flags) if (this & flags) != Noflags => str }.mkString(",")
}
...
object NodeFlags {
  ...
  def apply(str: String): NodeFlags = {
    ...
    reprValuePairs.foldLeft(Noflags) { case (res, (s, flags)) =>
      if (flagSet(s)) res | flags else res
    }
  }
}
